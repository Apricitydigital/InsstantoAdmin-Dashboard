...
import { SlotVisitSnapshot } from "@/components/slot-visit-snapshot"
import { UrgentSlotRequests } from "@/components/urgent-slot-requests"
...
import { categoryPath, DIAGNOSTICS, OUTCOMES, filterEvents, groupEvents, hubPath, REASONS, SlotUnavailabilityRepository, SOURCES, type EventFilters, type SlotUnavailabilityEvent } from "@/lib/queries/slot-unavailability"
...
const cards = [["", "Total recorded events"], ["no_schedules_found", "No schedules"], ["no_partner_for_selected_hub", "No partner in hub"], ["no_available_time_entries", "Unavailable time entries"], ["all_slots_inside_lead_time", "Lead-time restrictions"], ["service_slots_disabled", "Monthly slots disabled"]]
const definitions: [keyof EventFilters, string][] = [["serviceCoverageCityId", "City"], ["categoryPath", "Service category"], ["hubPath", "Hub"], ["subCategoryId", "Service subcategory"], ["reasonCode", "Reason"], ["source", "Source"], ["platform", "Platform"], ["outcome", "Visit outcome"]]
...
export default function SlotAvailabilityPage() {
  ...
  function options(key: keyof EventFilters) {
    ...
    for (const e of events) {
      ...
      const label = key === "serviceCoverageCityId" ? e.cityName : key === "categoryPath" ? `${e.categoryName} (${e.cityName})` : key === "hubPath" ? `${e.hubName} (${e.cityName} / ${e.categoryName})` : key === "subCategoryId" ? e.serviceName : key === "reasonCode" ? reason(e.reasonCode) : key === "source" ? SOURCES[e.source] || e.source : key === "outcome" ? OUTCOMES[e.outcome] || e.outcome : e.platform
      if (value) values.set(value, label)
    }
    if (key === "reasonCode") for (const [k, v] of Object.entries(REASONS)) values.set(k, v)
    if (key === "outcome") for (const [k, v] of Object.entries(OUTCOMES)) values.set(k, v)
    if (key === "source") for (const [k, v] of Object.entries(SOURCES)) values.set(k, v)
    return [...values.entries()].sort((a, b) => a[1].localeCompare(b[1]))
  }
  return <div className="space-y-6 p-4 md:p-6">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-2xl font-bold">Slot Visits & Availability</h1><p className="mt-1 text-sm text-muted-foreground">Review the slots customers saw and whether they went back without booking.</p></div><Button variant="outline" disabled={loading} onClick={() => setRefresh(v => v + 1)}><RefreshCw className="mr-2 h-4 w-4" />Refresh</Button></div>
    <p className="rounded-lg border bg-muted/30 p-3 text-sm text-muted-foreground">Counts represent recorded events, not unique customers. Legacy unavailable events do not record whether the customer booked or went back. Customer details are shown when the event contains a customer reference or UID; older events may not identify a customer.</p>
    <UrgentSlotRequests />
    ...
    {loading ? <div role="status" className="flex items-center justify-center gap-3 p-12"><Loader2 className="h-5 w-5 animate-spin" />Loading availability events ({loaded.toLocaleString()} read)…</div> : error ? <div role="alert" className="rounded-lg border border-destructive p-5"><p className="font-semibold">Could not load availability issues</p><p className="mt-2 break-words text-sm">{error}</p><p className="mt-2 text-sm text-muted-foreground">If access is denied, check that the updated Firestore rules are deployed and your account has analytics access.</p><Button variant="outline" className="mt-3" onClick={() => setRefresh(v => v + 1)}>Retry</Button></div> : <>
      <div className="grid gap-4 sm:grid-cols-2"><Card><CardHeader><CardTitle>Back without booking</CardTitle></CardHeader><CardContent className="text-3xl font-bold">{filtered.filter(e => e.outcome === "back_unbooked").length}</CardContent></Card><Card><CardHeader><CardTitle>Booked visits</CardTitle></CardHeader><CardContent className="text-3xl font-bold">{filtered.filter(e => e.outcome === "booked").length}</CardContent></Card></div><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{cards.map(([code, label]) => <Card key={code}><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">{label}</CardTitle></CardHeader><CardContent><p className="text-3xl font-bold">{(code ? filtered.filter(e => e.reasonCode === code).length : filtered.length).toLocaleString()}</p></CardContent></Card>)}</div>
      {!filtered.length ? <div className="rounded-lg border border-dashed p-12 text-center"><h2 className="font-semibold">No availability issues found</h2><p className="mt-2 text-sm text-muted-foreground">Try another date range or clear the filters. Visit snapshots appear after the customer app sends visit tracking events. Existing unavailable events remain visible.</p></div> : <>
        <Card><CardHeader><CardTitle>Hub / service summary</CardTitle><p className="text-sm text-muted-foreground">Highest recorded event counts first. Diagnostic counters are summed across events.</p></CardHeader><CardContent><Table><Headers values={["City", "Hub", "Service", "Attempts", "Most common reason", "Last occurrence", "Schedules fetched", "Hub mismatches", "Lead-time exclusions"]} /><TableBody>{grouped.map(g => <TableRow key={g.key}><TableCell>{g.event.cityName}</TableCell><TableCell>{g.event.hubName}</TableCell><TableCell>{g.event.serviceName}</TableCell><TableCell className="font-semibold">{g.count}</TableCell><TableCell>{reason(Object.entries(g.reasons).sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))[0][0])}</TableCell><TableCell>{time(g.last)}</TableCell><TableCell>{g.schedules}</TableCell><TableCell>{g.mismatches}</TableCell><TableCell>{g.leadTime}</TableCell></TableRow>)}</TableBody></Table></CardContent></Card>
        <Card><CardHeader><CardTitle>Detailed events</CardTitle><p className="text-sm text-muted-foreground">{filtered.length.toLocaleString()} attempts · Most recent first</p></CardHeader><CardContent><Table><Headers values={["Date / time", "Visit outcome", "Customer", "City", "Hub", "Service", "Source", "Platform", "Reason", "Diagnostics"]} /><TableBody>{filtered.slice(current * 100, (current + 1) * 100).map(e => <TableRow key={e.id}><TableCell>{time(e.createdAt)}</TableCell><TableCell>{OUTCOMES[e.outcome] || e.outcome}</TableCell><TableCell><p className="font-medium">{e.customerName || (e.customerUid ? "Unnamed customer" : "Not recorded")}</p>{e.customerUid && <p className="text-xs text-muted-foreground break-all">{e.customerUid}</p>}{e.customerPhone && <p className="text-sm">{e.customerPhone}</p>}{e.customerEmail && <p className="text-sm">{e.customerEmail}</p>}{e.customerUid && e.customerStatus !== "Available" && <p className="text-xs text-muted-foreground">{e.customerStatus}</p>}{e.customerUid && hasPermission("customers:view") && <Link className="text-sm text-primary underline" href={"/customers/" + encodeURIComponent(e.customerUid)}>View customer</Link>}</TableCell><TableCell>{e.cityName}</TableCell><TableCell>{e.hubName}</TableCell><TableCell>{e.serviceName}</TableCell><TableCell>{SOURCES[e.source] || e.source || "Unknown"}</TableCell><TableCell>{e.platform || "Unknown"}</TableCell><TableCell>{reason(e.reasonCode)}</TableCell><TableCell><p className="text-xs text-muted-foreground">Schedules: {e.diagnostics.schedulesFetched ?? "—"} · Hub mismatches: {e.diagnostics.hubMismatchExcluded ?? "—"} · Lead time: {e.diagnostics.leadTimeExcluded ?? "—"}</p><Button variant="link" className="px-0" onClick={() => setSelected(e)} aria-label={`View diagnostics for event ${e.id}`}>View details</Button></TableCell></TableRow>)}</TableBody></Table><div className="mt-4 flex items-center justify-between gap-3"><Button variant="outline" disabled={current === 0} onClick={() => setPage(current - 1)}>Previous</Button><span className="text-sm">Page {current + 1} of {pages}</span><Button variant="outline" disabled={current + 1 >= pages} onClick={() => setPage(current + 1)}>Next</Button></div></CardContent></Card>
      </>}
    </>}
    <Dialog open={!!selected} onOpenChange={open => { if (!open) setSelected(null) }}><DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>Slot visit details</DialogTitle><DialogDescription>Customer details, recorded metadata, and diagnostic counters for this occurrence.</DialogDescription></DialogHeader>{selected && <>
      <SlotVisitSnapshot event={selected} /><dl className="grid grid-cols-2 gap-3 text-sm">{Object.entries({ "Visit outcome": OUTCOMES[selected.outcome] || selected.outcome, "Visit ID": selected.visitId, "Opened at": time(selected.openedAt), "Snapshot captured": time(selected.snapshotAt), "Service timezone": selected.timeZone, "Customer name": selected.customerName, "Customer UID": selected.customerUid, "Customer reference": selected.customerPath, "Phone": selected.customerPhone, "Email": selected.customerEmail, "Customer status": selected.customerStatus, "Event ID": selected.id, "Event type": selected.eventType, "Version": selected.eventVersion, "Server time": time(selected.createdAt), "Device time": time(selected.clientCreatedAt), "City": selected.cityName, "City ID": selected.serviceCoverageCityId, "Category": selected.categoryName, "Coverage category ID": selected.serviceCoverageCategoryId, "Original category ID": selected.categoryId, "Hub": selected.hubName, "Hub ID": selected.serviceHubId, "Service": selected.serviceName, "Subcategory ID": selected.subCategoryId, "Source": SOURCES[selected.source] || selected.source, "Platform": selected.platform, "Reason code": selected.reasonCode, "Days searched": selected.daysSearched }).map(([label, value]) => <div key={label} className="min-w-0"><dt className="text-muted-foreground">{label}</dt><dd className="break-words font-medium">{value === "" ? "Not recorded" : value}</dd></div>)}</dl>
